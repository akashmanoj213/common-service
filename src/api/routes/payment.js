...
const { inititateTransaction, pushMessage, savePaymentInfo } = require("../../controllers/payment");
...
router.get('/test', async(req, res) => {
    try {
        res.status(200).json(success(res.statusCode, `Test message is succesfull`));
    } catch (error) {
        return res.status(500).json(error(res.statusCode, err.message));
    }
})
